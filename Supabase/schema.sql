-- MASTER SCHEMA VERSION - FINAL
-- Consolidated AutoWriter + SEO + Publisher Modules
-- Revised: 2026-01-20
...
CREATE TABLE IF NOT EXISTS public.article_styles (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  style_key character varying NOT NULL UNIQUE,
  name character varying NOT NULL,
  description text,
...
CREATE TABLE IF NOT EXISTS public.pre_articles (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
...
-- Triggers (Re-create to ensure existence)
...
-- ==========================================
-- 6. MIGRATION UPDATES & SEEDS (SAFE RUN)
-- ==========================================

-- Safely add missing columns to settings if table already existed
...
ALTER TABLE public.settings ADD COLUMN IF NOT EXISTS provider_google_enabled boolean DEFAULT true;
ALTER TABLE public.settings ADD COLUMN IF NOT EXISTS provider_anthropic_enabled boolean DEFAULT true;
ALTER TABLE public.settings ADD COLUMN IF NOT EXISTS image_mode character varying DEFAULT 'dalle3';

-- Seed Pricing Profiles (INSERT if not exists)
